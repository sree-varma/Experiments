...
def read_csv(filename,path):
    df=pd.read_csv(path+filename,dtype=None,delimiter=",")#,names=['ETA','PHI','PT','CHARGE'])
    gc.collect()
    return df
for i in range(1,21):
	...
	df_m=pd.DataFrame([[0]],columns=['MASS'])	

	df_im=read_csv(imFile1,pathim)

	df_mass=pd.read_csv(pathim+immass1,sep='\t',names=["MASS"])
	df_mass=df_m.append(df_mass,ignore_index=True)

	#df_im,df_mass=preprocessing.data(df_im,df_mass)
...
np.savez_compressed(path+signame+"_"+bgname+'/'+ptrange+'/'+"train_image_"+dataname+"_"+ptrange+"_"+R+"_"+method+".npz",image) 
np.savez_compressed(path+signame+"_"+bgname+'/'+ptrange+'/'+"train_mass_"+dataname+"_"+ptrange+"_"+R+"_"+method+".npz",mass)
